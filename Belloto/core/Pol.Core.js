...
var Pol = {  
  ...
  isArray     : function(val) { return val.constructor == Array; },
  ...
  toArray: function(o) {
    var items = [];
    Pol.each(o, function(e) { items.push(e) });
    return items;
  },
  ...
  show : function(e){    
    if(Pol.isArray(e)){
      e.forEach( function(el){ el.style.display = 'block'; })
    }else{
...
Pol.includes = [];
Pol.Core.Include = function(url, callback){
  if(Pol.includes.indexOf(url.toLowerCase())>-1){
    if(callback) callback();
    return;
  }
  Pol.includes.add(url.toLowerCase());
...
Pol.Core.Math.Rect.prototype.centerPoint = function() { return { x : this.left + (this.width >> 1), y : this.top + (this.height >> 1) };};
...
Pol.Core.Collision = {
  CreateCircle : function(x,y,radius){
    var _that = { x             : x, 
                  ...
                  distance      : function(p) { var dx = this.x - p.x; var dy = this.y - p.y; return Math.sqrt(dx * dx + dy * dy);}
    };   
    return _that
  },
