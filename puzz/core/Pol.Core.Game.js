...
Pol.Core.Game = function(){
  ...
  function __onResize(){
    ...
    var scaleToFitY = window.innerHeight / _that.GameHeight;        
    _that.CanvasRatio =  Math.min(scaleToFitX, scaleToFitY);
...
//    with($('MsgOverlay').style){      
//      width      = _that.GameWidth + 'px';
//      height     = _that.GameHeight + 'px';          
//      transform  =  'matrix({0}, 0, 0, {0}, {1}, {2})'.format(_that.CanvasRatio, 
//                                                              ((window.innerWidth - (_that.GameWidth / _that.CanvasRatio))) , 
//                                                              -_that.GameHeight / 8);
//    }
...
Pol.Core.Game.InputManager = function(){
  ...
  _that.BindEvents = function(target){
    ...
    function _normalizePosition(e) {
      ...
      return { x : Math.ceil( x * (1/Pol.Core.Game.CanvasRatio)), y : Math.ceil( y * (1/Pol.Core.Game.CanvasRatio))};
    }
...
Pol.Core.Game.Configuration = function(){
  ...
  _that.resetGame = function(){
    console.log('Pol.Core.Game.Configuration.resetGame()');
  }
...
Pol.Core.Math.Random = function(max,min){
  return Math.floor(Math.random()*(max-min+1)+min);
}

Pol.Core.Math.Clamp = function(value, min, max) {
